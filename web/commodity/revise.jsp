...
<%
    ...
    Commodity c = comTool.getCommodity(Integer.parseInt(cid));
    ...
    <div class="window-box">
        <div class="logo-line">
            <img
                    ...
                    alt=""
            />
            <div class="title-text">修改商品信息</div>
        </div>
        <div class="line"></div>
        <div class="revise-container">
            <div class="left-content">
                <div class="now-img-text">现在的封面：</div>
                <img class="now-img" src="<%=c.getImage()%>" alt="">
            </div>
...
<script>
    ...
    function send() {
        if ((url = '' && i == false)) {
            ...
            $.ajax({
                method: 'POST',
                url: 'http://localhost:8080/shopping/commodity/update',
                data: {
                    id: <%=c.getId()%>,
                    name: $('#name-input').val(),
                    desc: $('#desc-input').val(),
                ...
                success: function (res) {
                    ...
                    if (res == '200') {
                        ...
                        window.location.href =
                            'http://localhost:8080/shopping/commodity/mycommodity.jsp'
                    } else {
                        alert('修改失败')
                    }
                },
